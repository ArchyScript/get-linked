<template>
  ...
  >
    <div class="h-9 w-9 rounded-full ring-1 ring-secondary-500">
      ...
      <Avatar name="img-1" customClass="!w-full !h-full rounded-full" :alt="chat.username" />
    </div>
    ...
    >
      <div class="space-x-5 flex items-center truncate">
        <TypoHeaderText size="base" customClass="flex-1 !text-grey-500 truncate  ">
          {{ chat.username }}
        </TypoHeaderText>

        <TypoNormalText customClass="!text-grey-300 capitalize !text-[10px] ">
          {{ chat.date }}
        </TypoNormalText>
      </div>

      <div class="space-x-2 flex items-center truncate">
        <TypoNormalText size="sm" customClass="flex-1 !text-grey-400 truncate leading-[160%] ">
          {{ chat.message }}
        </TypoNormalText>

        <TypoNormalText
          v-if="chat.unreadMessages >= 1"
          customClass="!bg-primary-500 !text-primary-50 !h-5 !w-5 capitalize !text-[10px]  rounded-full flex justify-center items-center"
        >
          {{ chat.unreadMessages }}
        </TypoNormalText>
      </div>
